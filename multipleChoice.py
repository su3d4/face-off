"""Multiple choice prompt and image queries for facial expression recognition."""
...
def process_images_with_prompts(
    ...
    write_log_file(
        config,
        [
            "Starting multiple choice query processing...",
            f"Using model: {model}",
...
def main() -> None:
    """Main function to process multiple choice queries."""
    config = create_app_config()

    logger.info("Starting multiple choice query processing pipeline...")
    ...
    if image_paths and prompt_paths:
        ...
        # Process using the qwen3 model
        results = process_images_with_prompts(
            list(pairs.keys()), list(pairs.values()), config.ollama.model_qwen3_vl
        )
