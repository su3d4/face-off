...
def process_images_with_model(image_paths: list, model: str, prompt: str) -> None:
    ...
    log_messages = [f"Starting image processing with model: {model}"]
    log_messages.append(f"Images to process: {len(image_paths)}")
    ...
    log_messages.append("Image processing completed successfully")
...
def main() -> None:
    ...
    write_log_file(
        config,
        [
            "Starting facial expression recognition project...",
    ...
    if images:
        ...
        process_images_with_model(images, config.ollama.model_qwen3_vl, prompt)
        log_messages.append(f"Using model: {config.ollama.model_qwen3_vl}")
        write_log_file(config, log_messages)
    else:
